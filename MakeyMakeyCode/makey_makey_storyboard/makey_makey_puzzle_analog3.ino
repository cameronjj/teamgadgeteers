...
int pins[6]; //holds analogRead from pins
...
void pinRead(); //puzzle
void resCheck(); //puzzle
void checkFirst(); //puzzle
void checkState(); //puzzle
void printKeys(); //puzzle
...
void resCheck(){
  for(int i=0; i<6; i++){
    if(pins[i]>0){
      for(int j=0; j<6; j++){
        if(abs((R0*(1023.0/pins[i]-1))-resValue[j])<100){
          pieces[i]=j;
        }
      }
    }
